...
namespace Emgu_ObjectTracking.Tracking
{
    ...
    class TestMultiTracker
    {
        ...
        VectorOfRect _VectorOfRect = new VectorOfRect();
        public void Tracking(UMat img_curr)
        {
            ...
            if (isInit)
            {
                ...
                foreach (var bbox in objRecs)
                {
                    tracker.Add(frame, bbox);
                }

                _VectorOfRect.Push(objRecs.ToArray());
                isInit = false;
                return;
            }
            // Update tracking results
            //tracker->update(frame, bbox);
            tracker.Update(frame, _VectorOfRect);

            // Draw bounding box
            //rectangle(frame, bbox, Scalar( 255, 0, 0 ), 2, 1 );
            for (int i = 0; i < _VectorOfRect.Size; i++)
            {
                var bbox = _VectorOfRect[i];
                CvInvoke.Rectangle(img_curr, bbox, new MCvScalar(255, 0, 0), 3);
            }
